...
/**
 * Methods for running the import batch for the content type 'noticia'.
 *
 * @package Drupal\city_connect_import\Utility\Batch\Batch\Types\Noticia
 */
class Noticia {

  /**
   * Process Content Type noticia.
   *
   * @param array $args
  ...
  public static function build(array $args, $line, $node) {
    ...
    try {
      if (!empty($node)) {
         ...
         * @var \Drupal\Node\Entity\Node $new_node
         */
        $new_node = $entity_type_manager->getStorage('node')->create(['type' => 'noticia']);
      ...
      if ($new_node) {
        ...
        if ($args['uid']) {
          $new_node->set('field_autor', $args['uid']);
        }
        if ($args['promote']) {
        ...
        if ($args['field_e_destaque']) {
          $new_node->set('field_e_destaque', $args['field_e_destaque']);
        }        
        if ($args['langcode']) {
